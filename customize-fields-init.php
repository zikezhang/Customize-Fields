...
class customFieldTemplateInit
{
    ...
    function custom_field_template_init()
    {
        ...
        if (is_user_logged_in()) {
            if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'custom-field-template/customize-fields-template.php') {
    ...
    function custom_field_template_admin_menu() {
        add_options_page(__('Customize Fields', $this->base), __('Customize Fields', $this->parent->_token), 'manage_options', $this->parent->_token, array(&$this, 'custom_field_template_admin'));
    }
    ...
    function custom_field_template_selectbox() {
        ...
        $out .= ' var cftloading_select = function() {jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&id=\'+jQuery(\'#custom_field_template_select\').val()+\'&post=\'+jQuery(\'#post_ID\').val(), success: function(html) {';
        ...
        if ( !empty($options['custom_field_template_use_autosave']) ) :
            ...
            $out .= 'jQuery.ajax({type: \'POST\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxsave&post=\'+jQuery(\'#post_ID\').val()+\'&custom-field-template-verify-key=\'+jQuery(\'#custom-field-template-verify-key\').val()+\'&\'+fields, success: cftloading_select});';
        else :
...
<div class="dbx-content">';
    ...
    jQuery("#edButtonPreview").trigger("click"); }' . "\n";*/
                ...
                if ( !empty($val['category']) ) :
                    ...
                    foreach($categories as $cat_id) :
                        if ( is_numeric($cat_id) ) :
                            ...
                            $out .=		'jQuery(\'#in-'.$category_taxonomy[$cat_id].'-' . $cat_id . '\').click(function(){if(jQuery(\'#in-'.$category_taxonomy[$cat_id].'-' . $cat_id . '\').attr(\'checked\') == true) { if(tinyMCEID.length) { for(i=0;i<tinyMCEID.length;i++) {tinyMCE.execCommand(\'mceRemoveControl\', false, tinyMCEID[i]);} tinyMCEID.length=0;}; jQuery.get(\'?page=custom-field-template/customize-fields-template.php&cft_mode=selectbox&post=\'+jQuery(\'#post_ID\').val()+\'&\'+jQuery(\'#'.$taxonomy.'-all :input\').fieldSerialize(), function(html) { jQuery(\'#cft_selectbox\').html(html);';
                            if ( !empty($options['custom_field_template_use_autosave']) ) :
                                $out .= ' var fields = jQuery(\'#cft'.$suffix.' :input\').fieldSerialize();';
                                $out .= 'jQuery.ajax({type: \'POST\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxsave&post=\'+jQuery(\'#post_ID\').val()+\'&custom-field-template-verify-key=\'+jQuery(\'#custom-field-template-verify-key\').val()+\'&\'+fields, success: function(){jQuery(\'#custom_field_template_select\').val(\'' . $key . '\');jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&id=' . $key . '&post=\'+jQuery(\'#post_ID\').val(), success: function(html) {';
                                if ( !empty($options['custom_field_template_replace_the_title']) ) :
                                    $out .= 'jQuery(\'#cftdiv'.$suffix.' h3 span\').text(\'' . $options['custom_fields'][$key]['title'] . '\');';
                                endif;
                                $out .= 'jQuery(\'#cft\').html(html);}});}});';
                            else :
                                $out .=		'	jQuery(\'#custom_field_template_select\').val(\'' . $key . '\');jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&id=' . $key . '&post=\'+jQuery(\'#post_ID\').val()+\'&\'+jQuery(\'#'.$taxonomy.'-all :input\').fieldSerialize(), success: function(html) {';
                                if ( !empty($options['custom_field_template_replace_the_title']) ) :
                                    $out .= 'jQuery(\'#cftdiv'.$suffix.' h3 span\').text(\'' . $options['custom_fields'][$key]['title'] . '\');';
                                endif;
                                $out .= 'jQuery(\'#cft\').html(html);}});';
                            ...
                            $out .=		'	}else{ jQuery(\'#cft\').html(\'\');jQuery.get(\'?page=custom-field-template/customize-fields-template.php&cft_mode=selectbox&post=\'+jQuery(\'#post_ID\').val()+\'&\'+jQuery(\'#'.$taxonomy.'-all :input\').fieldSerialize(), function(html) { jQuery(\'#cft_selectbox\').html(html); jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&post=\'+jQuery(\'#post_ID\').val()+\'&\'+jQuery(\'#'.$taxonomy.'-all :input\').fieldSerialize(), success: function(html) { jQuery(\'#cft\').html(html);}}); });';
                            if ( !empty($options['custom_field_template_replace_the_title']) ) :
        ...
        if ( empty($options['custom_field_template_deploy_box']) && 0 != count( get_page_templates() ) ):
            ...
            $out .=	'jQuery(\'#page_template\').change(function(){ if(tinyMCEID.length) { for(i=0;i<tinyMCEID.length;i++) {tinyMCE.execCommand(\'mceRemoveControl\', false, tinyMCEID[i]);} tinyMCEID.length=0;}; jQuery.get(\'?post_type='.$_REQUEST['post_type'].'&page=custom-field-template/customize-fields-template.php&cft_mode=selectbox&post=\'+jQuery(\'#post_ID\').val()+\'&page_template=\'+jQuery(\'#page_template\').val(), function(html) { jQuery(\'#cft_selectbox\').html(html); jQuery.ajax({type: \'GET\', url: \'?post_type='.$_REQUEST['post_type'].'&page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&page_template=\'+jQuery(\'#page_template\').val()+\'&post=\'+jQuery(\'#post_ID\').val(), success: function(html) { jQuery(\'#cft\').html(html);';
            if ( !empty($options['custom_field_template_replace_the_title']) ) :
        ...
        if ( empty($options['custom_field_template_disable_initialize_button']) ) :
            ...
            $out .= 'if(confirm(\''.__('Are you sure to reset current values? Default values will be loaded.', 'custom-field-template').'\')){if(tinyMCEID.length) { for(i=0;i<tinyMCEID.length;i++) {tinyMCE.execCommand(\'mceRemoveControl\', false, tinyMCEID[i]);} tinyMCEID.length=0;};jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&default=true&id='.$suffix3.'&post=\'+jQuery(\'#post_ID\').val(), success: function(html) {';
        ...
        if ( empty($options['custom_field_template_disable_save_button']) ) :
            ...
            $out .= 'jQuery.ajax({type: \'POST\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxsave&post=\'+jQuery(\'#post_ID\').val()+\'&custom-field-template-verify-key=\'+jQuery(\'#custom-field-template-verify-key\').val(), data: fields, success: function() {jQuery(\'.delete_file_checkbox:checked\').each(function() {jQuery(this).parent().parent().remove();});}});';
            $out .= '" class="button" style="vertical-align:middle;" />';
        endif;
        $out .= '</div>';
    ...
    function add_quick_edit_custom_box($column_name, $type) {
        if( $column_name == 'custom-fields' ) {
            ...
            if ( count($options['custom_fields'])>1 ) {
                ...
                $out .= '</select>';
                $out .= '<input type="button" class="button" value="' . __('Load', 'custom-field-template') . '" onclick="var post = jQuery(this).parent().parent().parent().parent().attr(\'id\').replace(\'edit-\',\'\'); var cftloading_select = function() {jQuery.ajax({type: \'GET\', url: \'?page=custom-field-template/customize-fields-template.php&cft_mode=ajaxload&id=\'+jQuery(\'#custom_field_template_select\').val()+\'&post=\'+post, success: function(html) {jQuery(\'#cft\').html(html);}});};cftloading_select(post);" />';
            }
    ...
    function custom_field_template_admin() {
        ...
        require_once( 'customize-fields-router.php' );
